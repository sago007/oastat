...
cat oastat_create_tables_common.sql | grep -v "ALTER TABLE" | grep -v "ADD CONSTRAINT" | grep -v "RESTRICT2MYSQL" | grep -v "RESTRICT2POSTGRESQL" | sed -e 's/SERIAL/INTEGER/g' -e "s/now()/date('now')/g" > oastat_create_tables_sqlite.sql
